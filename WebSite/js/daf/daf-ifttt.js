...
(function () {
    var app,
        ...
        systemDisplayProperties = [], // this array can be used to extend the future complex "system" properties such "image" or "html" when a literal value is not enough
    ...
    function kioskDisplay(options) {
        ...
        _displayContent = div('app-display-content').attr('tabindex', 0).appendTo(_display);
        ...
        var physicalWidth = Math.min(Math.max(_kiosk.width(), _touch.toWidth(logicalWidth)), uiAutomationSettings('kiosk.maxWidth') || _touch.toWidth('xxl')),
            displayContentScrollbarInfo = scrollbarSize(_displayContent);

        _display.css({ width: physicalWidth, height: _display.height(), left: '50%', 'margin-left': -physicalWidth / 2 + 1 });
        _displayContent.css('width', physicalWidth + displayContentScrollbarInfo.width/* displayContent[0].offsetWidth - displayContent[0].clientWidth + 1*/);
        _displayContent.css('height', _display.height() + displayContentScrollbarInfo.height/* displayContent[0].offsetHeight - displayContent[0].clientHeight + 1*/);

        if (typeof displayConfig == 'string')
            div('app-content app-text').appendTo(_displayContent).html(displayConfig);
        else {
            if (!_display.data('created')) {
                _display.data('created', new Date());
            ...
            var dataView = appDataView(),
                currentContainer = _displayContent,
                currentRow, currentTabHeader, currentTab, currentTabText, currentTabContent;
            // update the layout of dislay configuration
            configUpdateLayout(displayConfig, _display);
            displayConfig.forEach(function (config, index) {
                ...
                if (config._newRow)
                    currentRow = div('app-row').appendTo(_displayContent);
                if (config._newCol)
                    currentContainer = setWidth(div('app-column'), config._width).appendTo(currentRow);
                var appendTo = config.append ? currentContainer.find('.app-container:last') : null,
                    container = appendTo ?
                        appendTo.length ? appendTo : currentContainer :
                        div('app-container').attr('data-index', index).appendTo(currentContainer).data(config),
                    content,
                    containerHeader,
                ...
                if (inDataView) {
                    ...
                    if (config._propCount) {
                        ...
                        if (gapContent)
                            gapContent.css({ 'max-width': container.width(), 'max-height': _displayContent.height() - displayContentScrollbarInfo.height });
                    }
    ...
    function containerContentAutoSize() {
        var autoSizeList = _displayContent.find('.app-container-autosize:not(.app-hidden)'),
            displayBottom,
            lastDisplayItem;
        if (autoSizeList.length) {
            autoSizeList.each(function () {
                var container = $(this),
                    ...
                    fixedHeight;
                if (!content.data('sized') && !content.is('.app-hidden')) {
                    if (displayBottom == null) {
                        displayBottom = _display.offset().top + _display.height();
                        lastDisplayItem = _displayContent.children().last();
                    }
    ...
    function kioskGapSize(config) {
        ...
        return gapSize == null ? 12 : gapSize;
    }
        ...
        if (hasColumns)
            rows.forEach(function (r) {
                var fixedWidth = 0,
                    autoWidthColCount = 0,
                    r0c0 = r[0][0];
                if (r.length === 1 && r[0].length === 1 && isGap(r0c0)) {
                        ...
                        if (w != null) {
                            if (typeof w == 'string')
                                if (w.match(/%$/))
                                    ...
                                    if (!layoutGridInfo) {
                                        var tempContainer = $('<div class="app-container"><div data-type="button"><div class="app-button app-no-text"></div></div></div>').appendTo(_display),
                                    ...
                                    w = parseInt(w);
                            c[0]._width = w = w <= 1 ? w : w / availWidth;
                            fixedWidth += w;
                        }
                        else {
                            autoWidthColCount++;
        ...
        displayConfig.forEach(function (config, index) {
            ...
            if (!changesDetected || dataViewChangeMonitor.indexOf(inDataView._id) >= 0)
                if (configIsList(config)) {
                    ...
                    data = inDataView.extension().commandRow() ? data : null;
